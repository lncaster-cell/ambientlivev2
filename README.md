...
Маршрут строится **один раз при активации области** (`AL_CacheAreaRoutes`) и
дальше берётся из локальных данных области (`al_route_<tag>_<idx>` + dense-индексы).
Система берёт максимум **10 точек на маршрут** в порядке заранее сохранённого списка.
