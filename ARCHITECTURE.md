...
- Никаких runtime-поисков по тегам/строкам (GetObjectByTag, GetNearestObjectByTag и т. п.) для управления жизнедеятельностью, кроме
  единоразового обхода области при активации для построения кеша маршрутов (в варианте глобального кеша — один раз на активацию, не на каждый запрос).
...
| Никаких runtime-поисков по тегам/строкам. | Маршруты и ссылки кешируются; обход области для маршрутов выполняется один раз при активации и не повторяется на каждом запросе. |
