...
* `AL_CacheRoutesForAllSlots` пытается кешировать маршруты по тегам, заданным на NPC (`alwp0..alwp5`), а при отсутствии локального тега использует fallback `AL_WP_S0..AL_WP_S5`.
...
Маршруты по слотам (через `alwp0..alwp5` и/или fallback `AL_WP_Sx`) кешируются, но в большинстве случаев **не используются**, так как активность не «требует» маршрута. В результате:

* лишнее кеширование вхолостую;
* вводящая в заблуждение конфигурация (ожидание, что `AL_WP_Sx` или `alwp<slot>` будет применяться для обычных активностей).

**Рекомендация:**

Определиться с контрактом:

* либо считать, что маршрут по `AL_WP_Sx`/`alwp<slot>` используется для **любой** активности, где маршрут существует;
* либо убрать кеширование маршрутов по слотам, если они не используются.
