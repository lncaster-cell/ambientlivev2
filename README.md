...
Если у waypoint’ов маршрута **есть** локальные индексы, то для **каждой**
точки нужно явно задать:

- `al_route_index` (0‑based индекс, без пропусков/дубликатов);
- `al_route_index_set` = 1 (флаг, что индекс задан).

Если **ни у одного** waypoint’а с этим тегом нет `al_route_index_set`,
система соберёт маршрут в простом режиме — в порядке обхода объектов.

Если хотя бы один waypoint с этим тегом имеет `al_route_index_set`,
то все точки без него считаются неподготовленными и пропускаются
с сообщением `missing al_route_index; skipped`.
