...
4. Получение активности по слоту (с переопределением от waypoint’а).
...
- сохраняет локации waypoint’ов по тегам:
  - `AL_WP_PACE`, `AL_WP_WWP`,
  - `AL_WP_S0..AL_WP_S5`.
...
Маршрут хранится на NPC в `r<slot>_*`. При смене активности или слота
выполняется `AL_RefreshRouteForSlot`, которая обновляет маршрут, если:

- поменялся тег маршрута (на основе активности), или
- маршрута ещё нет.

Маршрут по умолчанию:

- если активность требует конкретный тег (например, `AL_WP_PACE`),
  используется он;
- иначе используется маршрут по слоту `AL_WP_S<slot>`.

Если на текущем waypoint’е задан `al_activity`, он переопределяет
активность слота `a0..a5` для анимаций.

## Активности, роли и анимации

### Роли (role → activity)

Роль задаётся локальным int `al_role`. При первом спавне
`AL_ApplyRoleActivities` заполняет `a0..a5` активностью по умолчанию.
Повторное применение роли блокируется `al_role_applied`.
Если у NPC уже заданы `a0..a5` (любые значения > 0), роль не перезаписывает
эти слоты, чтобы сохранить пользовательскую настройку.

| Role ID | Роль | Activity ID |
| --- | --- | --- |
| 1 | Bartender | 42 (`AL_ACT_NPC_BARTENDER`) |
| 2 | Barmaid | 41 (`AL_ACT_NPC_BARMAID`) |
| 3 | Light Keeper | 40 (`AL_ACT_NPC_LIGHT_KEEPER`) |
| 4 | Smith | 15 (`AL_ACT_NPC_FORGE`) |
| 5 | Cook | 10 (`AL_ACT_NPC_COOK`) |
| 6 | Musician | 16 (`AL_ACT_NPC_GUITAR`) |
...
| 26 | TrainerPace | — | — | `AL_WP_PACE` |
| 27 | WWP | kneelidle, lookleft, lookright | — | `AL_WP_WWP` |
...
- **TrainerPace**: требует маршрут `AL_WP_PACE`.
- **WWP**: требует маршрут `AL_WP_WWP`.
...
1. В свойствах NPC задайте локальный int `al_role`.
2. Система заполнит `a0..a5` активностью по умолчанию.

### Вариант B — вручную через слоты

1. В свойствах NPC задайте локальные int `a0..a5`.
2. Каждое значение — ID активности из таблицы выше.
3. Активный слот определяется временем суток (`al_slot` на области, 0–5),
   поэтому для проверки убедитесь, что меняете слот текущего времени.
4. Если `a1..a5` не заданы (0), система использует значение `a0` как
   значение по умолчанию для остальных слотов.

### Настройка маршрутов

- Общие маршруты по слотам: `AL_WP_S0..AL_WP_S5`.
- Специальные маршруты:
  - `AL_WP_PACE` (TrainerPace)
  - `AL_WP_WWP` (WWP)
...
- **Маршруты по слотам** кешируются, но при текущей логике активность
  ставит маршрут только если он требуется (Pace/WWP). Поэтому `AL_WP_Sx`
  может не использоваться большинством активностей, если не требуется
  маршрут по тегу активности.
