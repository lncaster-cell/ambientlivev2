...
- отсутствие runtime-поисков по тегам/строкам для жизнедеятельности, кроме
  единоразового обхода области при активации для построения кеша маршрутов.
...
- Никаких runtime-поисков по тегам/строкам (GetObjectByTag, GetNearestObjectByTag)
  для управления поведением, кроме единоразового обхода области при активации
  для построения кеша маршрутов (в варианте глобального кеша — один раз на
  активацию, не на каждый запрос).
...
Если активность требует маршрут/партнёра/пару и условие не выполнено, система
применяет `AL_ACT_NPC_ACT_ONE`. При этом маршрут очищается
(`AL_ClearActiveRoute` в `al_npc_onud.nss`), и NPC не будет двигаться.
...
- При варианте с глобальным кешем обход маршрутов выполняется один раз при
  активации области, а не на каждом запросе.
- Система не использует heartbeat на NPC — это ожидаемое поведение.
- Корректность `al_player_count` зависит от штатных событий
  **OnEnter/OnExit/OnClientLeave**.
- При скриптовых перемещениях без этих событий необходимо вручную
  синхронизировать счётчики или вызывать соответствующие события.
